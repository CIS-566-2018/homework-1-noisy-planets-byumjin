...
void main()
{
    ...
    float timeSeed = u_TimeInfo.x * 0.01;

    vec2 freqScale = vec2(17.0 , 0.06);

    if(u_TimeInfo.y == 1.0)
    {
        vertexPos.xyz += vec3(sin(vertexPos.x * freqScale.x + timeSeed), cos(vertexPos.y * freqScale.x + timeSeed), sin(-vertexPos.y * freqScale.x + timeSeed)) * freqScale.y;
    }
